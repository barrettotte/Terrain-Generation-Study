...
namespace LibNoise.Unity
{
    ...
    public class Noise2D : IDisposable{
        ...
        private double GeneratePlanar(double x, double z){
			return this.m_generator.GetValue(x, 0.0, z);
        }
        ...
        public float[,] GeneratePlanar(double left, double right, double bottom, double top, bool seamless){

			float[,] sphericalData = new float[m_width, m_height];

            if (right <= left || top <= bottom || this.m_generator == null){
                throw new ArgumentException();
            }
            double xe = right - left;
            double ze = top - bottom;
        ...
        private double GenerateSpherical(double lat, double lon){
            ...
            return this.m_generator.GetValue(
				Math.Cos(Utils.DegToRad * lat) * Math.Cos(Utils.DegToRad * lon),
				Math.Sin(Utils.DegToRad * lat),
				Math.Cos(Utils.DegToRad * lat) * Math.Sin(Utils.DegToRad * lon));
        }
